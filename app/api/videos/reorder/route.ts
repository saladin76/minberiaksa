...
export async function POST(req: NextRequest) {
  try {
    ...
    if (videos.length > MAX_REORDER_ITEMS) {
      return NextResponse.json({ error: "عدد الفيديوهات كبير جدًا" }, { status: 400 });
    }
    ...
    await prisma.$transaction(
      updates.map(({ id, order }) => prisma.video.update({ where: { id }, data: { order } }))
    );
    ...
    queueAuditLog({
      ...
      messageAr: `${actor.actorName ?? "مسؤول"} أعاد ترتيب الفيديوهات (${updates.length})`,
      entityType: "Video",
    });

...
export async function POST(req: NextRequest) {
  try {
    ...
    if (reports.length > MAX_REORDER_ITEMS) {
      return NextResponse.json({ error: "عدد التقارير كبير جدًا" }, { status: 400 });
    }
    ...
    await prisma.$transaction(
      updates.map(({ id, order }) => prisma.report.update({ where: { id }, data: { order } }))
    );

    const actor = auditActorFromDashboardSession(session!);
    queueAuditLog({
      ...actor,
      action: "REPORT_REORDER",
      messageAr: `${actor.actorName ?? "مسؤول"} أعاد ترتيب التقارير (${updates.length})`,
      entityType: "Report",
    });

...
export async function POST(req: NextRequest) {
  try {
    ...
    if (stories.length > MAX_REORDER_ITEMS) {
      return NextResponse.json({ error: "عدد القصص كبير جدًا" }, { status: 400 });
    }
    ...
    await prisma.$transaction(
      updates.map(({ id, order }) => prisma.story.update({ where: { id }, data: { order } }))
    );

    const actor = auditActorFromDashboardSession(session!);
    queueAuditLog({
      ...actor,
      action: "STORY_REORDER",
      messageAr: `${actor.actorName ?? "مسؤول"} أعاد ترتيب القصص (${updates.length})`,
      entityType: "Story",
    });

...
export async function POST(req: NextRequest) {
  try {
    ...
    if (playlists.length > MAX_REORDER_ITEMS) {
      return NextResponse.json({ error: "عدد قوائم التشغيل كبير جدًا" }, { status: 400 });
    }
    ...
    await prisma.$transaction(
      updates.map(({ id, order }) => prisma.videoPlaylist.update({ where: { id }, data: { order } }))
    );
    ...
    queueAuditLog({
      ...
      messageAr: `${actor.actorName ?? "مسؤول"} أعاد ترتيب قوائم التشغيل (${updates.length})`,

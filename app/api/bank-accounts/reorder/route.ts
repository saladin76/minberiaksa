...
export async function POST(req: NextRequest) {
  try {
    ...
    if (bankAccounts.length > MAX_REORDER_ITEMS) {
      return NextResponse.json({ error: "عدد الحسابات كبير جدًا" }, { status: 400 });
    }
    ...
    await prisma.$transaction(
      updates.map(({ id, order }) => prisma.bankAccount.update({ where: { id }, data: { order } }))
    );

    const actor = auditActorFromDashboardSession(session!);
    queueAuditLog({
      ...actor,
      action: "BANK_ACCOUNT_REORDER",
      messageAr: `${actor.actorName ?? "مسؤول"} أعاد ترتيب الحسابات البنكية (${updates.length})`,
      entityType: "BankAccount",
    });

    return NextResponse.json({ ok: true });
  } catch (error) {
    console.error("Error reordering bankAccounts:", error);

...
export async function POST(req: NextRequest) {
  try {
    ...
    if (booklets.length > MAX_REORDER_ITEMS) {
      return NextResponse.json({ error: "عدد الكتيبات كبير جدًا" }, { status: 400 });
    }
    ...
    await prisma.$transaction(
      updates.map(({ id, order }) => prisma.booklet.update({ where: { id }, data: { order } }))
    );

    const actor = auditActorFromDashboardSession(session!);
    queueAuditLog({
      ...actor,
      action: "BOOKLET_REORDER",
      messageAr: `${actor.actorName ?? "مسؤول"} أعاد ترتيب الكتيبات (${updates.length})`,
      entityType: "Booklet",
    });

    return NextResponse.json({ ok: true });
  } catch (error) {
    console.error("Error reordering booklets:", error);

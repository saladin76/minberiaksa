...
export async function POST(req: NextRequest) {
  try {
    ...
    if (courses.length > MAX_REORDER_ITEMS) {
      return NextResponse.json({ error: "عدد الدورات كبير جدًا" }, { status: 400 });
    }
    ...
    await prisma.$transaction(
      updates.map(({ id, order }) => prisma.course.update({ where: { id }, data: { order } }))
    );

    const actor = auditActorFromDashboardSession(session!);
    queueAuditLog({
      ...actor,
      action: "COURSE_REORDER",
      messageAr: `${actor.actorName ?? "مسؤول"} أعاد ترتيب الدورات (${updates.length})`,
      entityType: "Course",
    });

...
export async function POST(req: NextRequest) {
  try {
    ...
    if (faqs.length > MAX_REORDER_ITEMS) {
      return NextResponse.json({ error: "عدد الأسئلة كبير جدًا" }, { status: 400 });
    }
    ...
    await prisma.$transaction(
      updates.map(({ id, order }) => prisma.faq.update({ where: { id }, data: { order } }))
    );
    ...
    queueAuditLog({
      ...actor,
      action: "FAQ_REORDER",
      messageAr: `${actor.actorName ?? "مسؤول"} أعاد ترتيب الأسئلة الشائعة (${updates.length})`,
      entityType: "Faq",
    });

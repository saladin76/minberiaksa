...
export function ProjectDonationPanel({ project, metric, suggestedAmounts, acceptsGift }: { project: ProjectRecord; metric?: ProjectMetric; suggestedAmounts?: number[]; acceptsGift: boolean }) {
  ...
  const selectMode = (nextMode: DonationMode) => {
    setMode(nextMode);
    setPrototypeMessage("");
    if (nextMode === "recurring" && recurringSupported) {
      setFrequency("شهري");
      setCustomAmount("");
      setAmount(metric?.unitAmount || baseAmounts[0] || 100);
    }
  };
  ...
  const addToBasket = () => {
    ...
    setToast(true); setPrototypeMessage("تمت إضافة التبرع التجريبي إلى سلة العطاء.");
    window.setTimeout(() => setToast(false), 2400);
  };

  const securePrototype = () => {
    if (!canSubmit) return;
    setPrototypeMessage("سيتم الانتقال إلى Checkout بعد ربط نظام الدفع. لا يتم تنفيذ دفع حقيقي الآن.");
  };

  const FormContent = ({ mobile = false }: { mobile?: boolean }) => <div className={mobile ? "donation-form donation-form--mobile" : "donation-form"}>
  ...
  return <>
    <aside className="project-donation-panel" aria-label="لوحة التبرع للمشروع"><div className="donation-panel-heading"><span>تبرع لهذا المشروع</span><h2>{project.title.ar}</h2><p>Frontend Prototype — لا يتم تنفيذ دفع حقيقي.</p></div><FormContent /></aside>

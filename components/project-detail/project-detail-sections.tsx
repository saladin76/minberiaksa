...
function ProjectHeroMedia({ project, visualType }: { project: ProjectRecord; visualType: ProjectVisualType }) {
  if (project.image) return <figure className="project-detail-hero-media"><img src={project.image.sourceUrl} alt={project.image.alt.ar} /></figure>;
        ...
        <div className="project-detail-shell">
          <div className="project-detail-primary">
            ...
            <div className="project-detail-content-grid">
              ...
              <section id="updates" className="project-editorial-section project-updates" aria-labelledby="updates-title"><div className="project-section-heading"><span>تحديثات المشروع</span><h2 id="updates-title">آخر ما نُشر</h2></div>{detail.updates.length ? <ol className="project-update-timeline">{detail.updates.map((update) => <li key={update.id}><div className="update-marker" aria-hidden="true" /><article><div className="update-meta"><span>{update.dateLabel}</span><b>{update.stage}</b><i>{updateStatusLabels[update.status]}</i></div><h3>{update.title}</h3><p>{update.description}</p>{update.mediaLabel ? <div className="update-media-placeholder">{update.mediaLabel}</div> : null}<small>المصدر: {update.sourceLabel}</small></article></li>)}</ol> : <div className="project-updates-empty"><h3>لا توجد تحديثات منشورة حاليًا</h3><p>ستظهر التحديثات هنا بعد مراجعتها واعتمادها.</p></div>}</section>
              <section id="proof" className="project-editorial-section project-proof" aria-labelledby="proof-title"><div className="project-section-heading"><span>تقارير المشروع</span><h2 id="proof-title">التقارير والوثائق المتاحة</h2></div><div className="project-proof-layout"><div className="project-proof-list">{detail.proofItems.map((item) => <article key={item.label}><span aria-hidden="true" /><div><h3>{item.label}</h3><strong>{proofStatusLabels[item.status]}</strong><p>{item.note}</p></div></article>)}</div><aside className="project-document-preview" aria-label="التقارير الميدانية"><span>التقارير الميدانية</span><h3>ننشر التقارير بعد اعتمادها</h3><p>يظهر كل تقرير هنا بعد مراجعته وربطه بالمشروع.</p><a href="/impact">عرض التقارير والتحديثات</a></aside></div></section>

...
const visualTypeLabels: Record<ProjectVisualType, string> = {
  emergency: "إغاثة عاجلة",
  essentials: "غذاء ومياه",
  education: "تعليم ورعاية",
  alquds: "مشروع مقدسي",
  waqf: "وقف للقدس",
  recurring: "تبرع دوري",
};
...
function ProjectHeroMedia({ project, visualType }: { project: ProjectRecord; visualType: ProjectVisualType }) {
  ...
  return (
    <div className={`project-visual-fallback project-visual-fallback--${visualType}`} role="img" aria-label={`${project.title.ar} في ${regionLabels[project.region]}`}>
      <span>{regionLabels[project.region]}</span>
      <strong>{project.title.ar}</strong>
      <small>{visualTypeLabels[visualType]}</small>
      <i aria-hidden="true" />
    </div>
  );
}
            ...
            <section className="project-detail-hero" aria-labelledby="project-title">
              ...
              <div className="project-detail-hero-copy">
                <div className="project-detail-badges">
                  <span>{visualTypeLabels[visualType]}</span><b>{regionLabels[project.region]}</b><i>{urgent ? "عاجل" : statusLabels[project.status]}</i>
                </div>
                ...
                <div className="project-detail-quick-actions">
                  <a href="#donation-panel">تبرع الآن</a>
                  <a href="#updates">تحديثات المشروع</a>
                  <a href="#proof">عرض التقارير</a>
                </div>

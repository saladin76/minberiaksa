...
export function RecurringPlanBuilder({ projects }: { projects: ProjectRecord[] }) {
  ...
  const add = () => {
    ...
    window.dispatchEvent(new CustomEvent("minber:add-to-basket", { detail: {
      ...
      frequency, amount: value, currency: currency.code, receiptType: "recurring", available: true,
    }}));
  ...
  return <div className="simple-giving-builder" id="recurring-plan-builder">
    ...
    <section className="builder-review"><span>4</span><div><h3>مراجعة الاختيار</h3><dl><div><dt>المشروع</dt><dd>{project?.title.ar ?? "—"}</dd></div><div><dt>التكرار</dt><dd>{frequency}</dd></div><div><dt>المبلغ</dt><dd>{valid?`${value.toFixed(2)} ${currency.code}`:"—"}</dd></div></dl></div></section>

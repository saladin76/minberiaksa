...
import type { CmsCourse } from "@/lib/minbar/cms";
 ...
 */
export default function CoursesPage({ courses }: { courses: CmsCourse[] }) {
        ...
        <div style={{ maxWidth: 1240, margin: "0 auto", padding: "0 24px" }}>
          <div className="crs-grid" style={{ display: "grid", gridTemplateColumns: "repeat(3,minmax(0,1fr))", gap: 20 }}>
            {courses.map((course) => {
              ...
              /* A course with its own page links there (the Zengi course is the
                 one built); anything else is a recording that opens in place,
                 on a card that is still a real anchor to the video. */
              const internal = course.hasDetailPage;
              const href = internal
                ? (course.slug === "zenki" ? miaPath("zenkiCourse", locale) : miaPath("courses", locale))
                : course.externalUrl || (course.introVideoId ? `https://www.youtube.com/watch?v=${course.introVideoId}` : "");
              const isSeminar = course.kind === "SEMINAR";
                ...
                <>
                  <span style={{ position: "relative", display: "block", width: "100%", aspectRatio: "16/9", background: "var(--navy)", overflow: "hidden" }}>
                    ...
                    <span style={{ position: "absolute", insetInlineStart: 14, top: 12, display: "inline-flex", alignItems: "center", gap: 7, padding: "5px 11px", borderRadius: 999, background: "rgba(2,34,34,.62)", border: "1px solid rgba(211,154,39,.6)", color: "#F2D9A4", fontSize: 11, fontWeight: 900, letterSpacing: ".04em", backdropFilter: "blur(3px)" }}>
                      {t(isSeminar ? "kindSeminar" : "kindCourse")}
                    </span>
                    <span style={{ position: "absolute", insetInlineStart: 14, bottom: 12, display: "inline-flex", alignItems: "center", gap: 7, color: "#fff", fontSize: 11.5, fontWeight: 800, textShadow: "0 1px 6px rgba(2,34,34,.7)" }}>
                      ...
                      {t(course.unitsCount ? "unitsCount" : "recordedVideo")}
                    </span>
                  </span>
                  ...
                  <span style={{ display: "grid", gridTemplateRows: "auto 1fr auto", gap: 8, padding: "18px 20px 20px" }}>
                    ...
                    {course.description ? (
                      <span style={{ color: "var(--muted)", fontSize: 13, lineHeight: 1.85, alignSelf: "start", display: "-webkit-box", WebkitLineClamp: 3, WebkitBoxOrient: "vertical", overflow: "hidden" }}>
                        {course.description}
                      </span>
                    ) : (
                      <span />
                    ...
                    <span style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10, marginTop: 6, paddingTop: 14, borderTop: "1px solid var(--border)" }}>
                      <span style={{ display: "inline-flex", alignItems: "center", gap: 9, color: "var(--red)", fontWeight: 900, fontSize: 13.5 }}>
                        {t(internal ? "courseDetails" : "watchNow")}
              ...
              return internal ? (
                <Link key={course.id} href={href} className="crs-card" style={frame}>
                  {body}
                </Link>
              ) : (
                <a
                  key={course.id}
                  href={href}
                  target={course.externalUrl && !course.introVideoId ? "_blank" : undefined}

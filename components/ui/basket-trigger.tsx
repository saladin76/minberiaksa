...
type BasketItem = { id:number; projectId?:string; slug?:string; intent:BasketIntent; intentLabel:string; project:string; donationMode?:"one-time"|"recurring"|"gift"; frequency?:string; amount:number; currency:"USD"; giftData?:GiftData; dedicationData?:DedicationData };
...
const initialItems: BasketItem[] = [
  { id:1,intent:"zakat",intentLabel:"زكاة",project:"زكاة لفلسطين",donationMode:"one-time",amount:120,currency:"USD" },
  { id:2,intent:"waqf",intentLabel:"وقف",project:"سهم وقفي للقدس",donationMode:"one-time",amount:250,currency:"USD" },
...
export function BasketTrigger({ compact=false }:{ compact?:boolean }) {
  ...
  return <>
    ...
    {open?<div className="drawer-layer"><button className="drawer-backdrop" type="button" aria-label="إغلاق سلة العطاء" onClick={()=>setOpen(false)}/><aside className="basket-drawer" role="dialog" aria-modal="true" aria-labelledby="basket-title">
      ...
      {items.length===0?<div className="basket-empty"><BasketIcon width={34} height={34}/><h3>سلة العطاء فارغة</h3><p>أضف مشروعًا أو نية تبرع لتظهر هنا قبل المراجعة.</p><Button href="/projects" variant="outline" onClick={()=>setOpen(false)}>استكشف المشاريع</Button></div>:<div className="basket-items" aria-live="polite">{items.map(item=><article className={`basket-item basket-item--${item.intent}`} key={item.id}><div className="basket-item-top"><Badge tone={item.intent==="general"?"neutral":item.intent}>{item.intentLabel}</Badge><IconButton aria-label={`إزالة ${item.project}`} onClick={()=>setItems(current=>current.filter(entry=>entry.id!==item.id))}><TrashIcon width={17} height={17}/></IconButton></div><h3>{item.project}</h3><div className="basket-item-context"><span>{item.donationMode?modeLabels[item.donationMode]:"مرة واحدة"}</span>{item.frequency&&<span>{item.frequency}</span>}{item.giftData?.recipient&&<span>إهداء إلى: {item.giftData.recipient}</span>}{item.dedicationData?.ownerName&&<span>صاحب الوقف: {item.dedicationData.ownerName}</span>}</div><div className="basket-item-bottom"><strong>{item.amount} {item.currency}</strong><div className="amount-controls" aria-label={`تعديل مبلغ ${item.project}`}><IconButton aria-label="خفض المبلغ" onClick={()=>changeAmount(item.id,-1)}><MinusIcon width={16} height={16}/></IconButton><span>تعديل</span><IconButton aria-label="زيادة المبلغ" onClick={()=>changeAmount(item.id,1)}><PlusIcon width={16} height={16}/></IconButton></div></div></article>)}</div>}

...
type BasketItem = { id:number; intent:BasketIntent; intentLabel:string; project:string; amount:number; currency:"USD" };
type AddBasketDetail = Omit<BasketItem,"id">;

const initialItems: BasketItem[] = [
  { id:1,intent:"zakat",intentLabel:"زكاة",project:"زكاة لفلسطين",amount:120,currency:"USD" },
  { id:2,intent:"waqf",intentLabel:"وقف",project:"سهم وقفي للقدس",amount:250,currency:"USD" },
  { id:3,intent:"urgent",intentLabel:"إغاثة عاجلة",project:"إغاثة غزة",amount:75,currency:"USD" },
];
...
export function BasketTrigger({ compact=false }:{ compact?:boolean }) {
  ...
  useEffect(()=>{
    const addItem=(event:Event)=>{
      const detail=(event as CustomEvent<AddBasketDetail>).detail;
      if(!detail||!detail.project||!Number.isFinite(detail.amount)) return;
      setItems(current=>[...current,{...detail,id:Date.now()}]);
    };
    window.addEventListener("minber:add-to-basket",addItem);
    return()=>window.removeEventListener("minber:add-to-basket",addItem);
  },[]);
  ...
  return <>
    <button ref={triggerRef} type="button" className={`basket-trigger ${compact?"basket-trigger--compact":""}`} aria-label={`سلة العطاء، ${items.length} عناصر`} aria-haspopup="dialog" aria-expanded={open} onClick={()=>setOpen(true)}>
      <BasketIcon/>{!compact?<span>سلة العطاء</span>:null}<span className="basket-count" aria-hidden="true">{items.length}</span>
    </button>
    {open?<div className="drawer-layer">
      <button className="drawer-backdrop" type="button" aria-label="إغلاق سلة العطاء" onClick={()=>setOpen(false)}/>
      <aside className="basket-drawer" role="dialog" aria-modal="true" aria-labelledby="basket-title">
